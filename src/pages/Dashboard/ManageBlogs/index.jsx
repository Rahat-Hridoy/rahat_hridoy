...
const ManageBlogs = () => {
    ...
    const filteredBlogs = blogs
        ...
        .sort((a, b) => {
             if (sortBy === 'newest') return b.id - a.id; // Corrected timestamp sorting
            if (sortBy === 'oldest') return a.id - b.id; // Corrected timestamp sorting
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                     <h1 className="text-3xl font-semibold font-primary text-white mb-2">Manage Blogs</h1>
                    <p className="text-gray-400 font-second text-sm">Create, edit, and manage your blog posts</p>
                </div>
                <button
            ...
            {/* Blogs List */}
             {filteredBlogs.length > 0 ? (
                viewMode === 'list' ? (
                     <BlogList blogs={filteredBlogs} handleEdit={handleEdit} handleDelete={handleDelete} />
                ) : (
                    <BlogGridView blogs={filteredBlogs} handleEdit={handleEdit} handleDelete={handleDelete} />
                )
            ) : (
                <p className="text-gray-500 font-second text-center py-10 bg-cardBG rounded-xl border border-white/10">No blogs found matching your criteria.</p>
            )}
        </div>
    );
};
